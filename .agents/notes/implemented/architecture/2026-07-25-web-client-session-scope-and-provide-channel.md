...
- The create arm: on a miss, `session.create({workspaceId})` returns the new id. Success confirms Workspace attachment, so `WorkspaceManager` adds that id to the currently installed Workspace snapshot before `connectWorkspace` resolves. It preserves create-confirmed membership across unordered full snapshots until an explicit `host/session-removed` or `host/workspace-removed` frame; an in-flight `workspace.list` replays only the attachment delta, preserving newer baseline fields.
- An unknown workspaceId fails loud (never silently creating somewhere else).
- The resolution guarantee (one contract for both arms): when the promise resolves, the returned id is already in the Session list store and the target Workspace's `sessionIds`, and `sessions.binding(id)` resolves synchronously. A draft mover can therefore write text into the new scope's machine before open, and navigation never opens the member Session against a stale Workspace projection.
...
| Wait for `host/workspace-changed` before opening a created Session | The push frame is asynchronous, unordered against the unary response, and may be absent from a browser's first connection. The successful create response already confirms attachment; waiting leaves navigation without a bounded settlement point, while allowing an older full snapshot to erase the confirmation makes the blank composer inert until a refresh. |
