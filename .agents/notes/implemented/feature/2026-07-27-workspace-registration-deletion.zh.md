...
菜单、`Modal` 和按钮保留现有结构与设计 token。该 Workspace 对话框绝不删除会话；独立的破坏性会话生命周期记录在[会话删除](2026-08-21-session-deletion.md)中。

## 考虑过的替代方案

**级联删除会话。** 不予采纳，因为 Workspace 注册记录不拥有会话持久化，且产品需求是将历史记录保留在 Ungrouped 下。单独命名的会话删除生命周期负责自身的运行状态检查、后代对象语义和明确 UI。
