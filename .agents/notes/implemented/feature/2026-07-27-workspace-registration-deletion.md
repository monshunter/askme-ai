...
The menu, Modal, and buttons retain their existing structure and design tokens. This Workspace dialog never deletes Sessions; the separate destructive Session lifecycle is recorded in [Session deletion](2026-08-21-session-deletion.md).

## Alternatives considered

**Cascade-delete Sessions.** Rejected because Workspace registration does not own Session persistence and the product requirement is to preserve histories under Ungrouped. The separately named Session deletion lifecycle owns its running checks, descendant semantics, and explicit UI.
