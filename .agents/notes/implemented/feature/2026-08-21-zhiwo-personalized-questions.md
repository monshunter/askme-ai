...
Zhiwo also owns browser-document branding. The Host replaces the initial build title, generic fish favicon, install manifest, and manifest icon before serving the page. The Client supplies “知我AI” through the product-title waterfall and “AskmeAI | 知我AI” through the document-title waterfall, so blank and titled Sessions show only the bilingual tab copy after React mounts; the icon is the same rounded “知” mark used in the application shell. Generic Web profiles retain their build-selected title, favicon, and install metadata.
