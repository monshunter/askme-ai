...
Product 在现有 Connection `/api` Channel 上注册一个内部 `zhiwo/questions` 方法。知我AI访问策略允许该方法，并在分发前继续执行已有的 Visitor Session 所有权检查。问候页请求返回四个轮换问题，存在项目时通常是两个全局问题与两个具体项目问题。后续请求只在最新 Turn 以 `completed` 结束时受理，并严格返回两个由最近用户消息与 Assistant 消息推断的问题，以及两个从初始化全局问题池中选择的问题。上下文提到某个项目时，推断问题使用该项目名；否则使用用户最近问题中经过清理的短片段。请求携带已经展示过的 ID，因此自动更新与手动刷新会持续轮换，直到对应类别用尽。
...
知我AI 同时拥有浏览器 Document 品牌。Host 在返回页面前替换初始构建标题、通用鱼形 Favicon、安装 Manifest 及其图标；Client 通过 Product Title Waterfall 提供“知我AI”，让空白会话和已有标题会话在 React 挂载后继续保留该名称。图标使用应用外壳同款圆角“知”标记；通用 Web Profile 继续保留构建时选择的标题、图标与安装元数据。

## 隐私与生命周期

问题文本可以包含清理后的顶层项目名，但不得包含绝对路径、`userdata`、DSH 或 Harness 实现名、System Prompt、Tool Trace 或隐藏条目名。目录缓存使用 Workspace 路径哈希而不是路径本身，并采用私有目录与文件权限；缓存保存派生目录与指纹，不保存文档内容。缓存解析、浏览器请求与浏览器响应都是不受信 JSON 入口，必须拒绝未知版本、无效 Source Tag、超长字段、重复 ID 与错误类别数量。

扫描只发布完整目录 Revision，Disposer 会阻止插件销毁后才完成的异步任务替换状态。UI 在 Session、Locale 或组件变化时中止请求，并忽略过期结果。未以 `completed` 结束的 Turn 不得替换上一次成功的后续问题集。

## 考虑过的替代方案

**通过 LLM 生成所有提示问题。** 启动阶段没有 Session Log 可以承载模型可见的 Workspace 摘要，而且 Provider 离线时功能会不可用或增加成本，因此不采用。确定性目录与 Transcript Projection 不需要额外模型调用。
...
- 每次新完成的 Assistant Turn 都把面板替换为严格两个上下文问题与两个全局问题。失败、取消、阻塞、达到 Token 上限与中断的 Turn 保留上一次成功集合。
...
- 空白与已有标题的浏览器标签及安装元数据都使用“知我AI”和圆角“知”图标，且不含 DSH 产品文案或通用鱼形标记。
...
顶层目录名可能很短或偏技术，因此项目问题不一定与内容推断的问题同样自然。有界方案有意优先保证可预测的启动速度与隐私；未来若要从正文推断，必须另行设计模型可见输入的日志记录与失效机制。如果一次编辑同时保留了文件大小与修改时间，不读取正文就无法识别；缓存接受这一项窄范围文件系统限制。
