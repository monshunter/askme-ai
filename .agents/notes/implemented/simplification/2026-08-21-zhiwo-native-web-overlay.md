...
The product Patch selects that preset, pins read-only sandbox policy, removes unused configuration and coding occupants from the browser roster, disables the generic Workspace UI, Session-log download plugin, and local spill backend, and inserts `packages/zhiwo/ui`. The Host keeps its native directory-picker dependency, but the AskmeAI access policy does not expose its methods. The UI package fills native brand and hero-headline slots, projects the visitor's native Session store as a flat history list, uses the native Workspace and Session runtimes to connect a clean browser to the sole Workspace, removes the resident command/access-mode cluster, context meter, and Session statistics strip from presentation, and contributes a sidebar action for switching the native locale between Chinese and English. Chinese presents “知我AI”, while English presents “AskmeAI.” The blank greeting contains only the invitation to get to know the material owner and does not present the Agent as an independent subject. The overlay changes only the leading icon and title on native search, read, and reasoning rows. Native summaries, path links, expanded cards, reasoning bodies, document-preview actions, and inspection controls remain intact; the running tail alone receives product-specific copy. The packaged AskmeAI logo supplies the browser, install, and greeting surfaces; the expanded sidebar shows the localized wordmark without the logo, and the collapse control retains its accessible label without a visual tooltip. One same-origin watercolor image covers the entire frame behind both the sidebar and conversation. The product pins the native theme runtime to its only supported light palette, so system or stored dark preferences cannot mix dark tokens into the green-and-cream stylesheet. That stylesheet does not replace the native layout, composer, messages, or controls. Blank Sessions start with a compact icon rail; active Sessions expose the visitor's full history sidebar. Model selection, sending, and message feedback stay on the native runtimes, with feedback protected by the same visitor Session ownership checks. The UI package owns no transport, persisted state, or conversation implementation. The product-specific route and confinement rules are owned by the [document-preview decision](../bug-fix/2026-08-21-zhiwo-workspace-confinement-and-document-preview.md).
