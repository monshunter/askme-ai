...
Input Dock 会验证并呈现 Product 私有 Connection 方法返回的四个双语语义问题。空白 Session 显示确定性问候问题。每次新 Turn 完成后，Product 都会要求当前对话模型实时生成严格两个上下文问题，再与两个确定性全局问题组合。自动请求与手动“换一组”使用同一个已完成 Turn 端点和模型路径。点击问题只填充原生 Draft，不会直接发送。请求采用 Single-flight；刷新失败或响应格式错误时，原来的四个问题保持可用。切换语言只改变同一组语义 ID 的文本，不会再发请求。
