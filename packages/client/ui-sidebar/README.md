...
The expanded brand row renders `sidebar.brand.mark` and `sidebar.brand.name` as static identity slots, then renders the optional `sidebar.brand.action` single slot as an adjacent independent interaction. New Session remains the dedicated control below the identity. The collapsed rail renders only the same mark slot as its expand control. Without occupants, the shell uses the fish mark and a `DSH Local Build` label carrying the build's 7-character `DSH_CLIENT_COMMIT_HASH` badge, and leaves the action empty. A deployment package can replace the identity or provide a link or control without replacing the New Session behavior or rail geometry; declaration-aware `slots.inject()` lets such a package activate before or after the sidebar.
...
`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the declared brand identity/action, `sidebar.workspaces`, and `sidebar.settings` child slots, and injected `startSession` plus sidebar-toggle callbacks. There is no plugin store.
