...
`WorkspaceRuntime.openPath(path)` 会先把解析后的路径交给 `workspaces/open-path` 瀑布。产品 Listener 调用 `next()`，并在已通过自身受限界面打开路径时返回 `true`。若没有 Listener 接管，Runtime 才调用原生 `host.openPath` 操作；普通桌面行为因此保持不变，而不是让每个浏览器产品都获得这项 Host 能力。
