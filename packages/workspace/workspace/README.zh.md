...
- `ctx.workspaceRegistry.forgetSession(id)`：从每个 Workspace 记账和 `archivedSessionIds` 中移除一个已永久删除的会话，再使其缓存 header 与路径信息失效。该操作具有幂等性，且绝不触碰会话日志或文件系统；这些操作由删除所有者执行。
...
- 破坏性文件夹移除仍未提供。删除 Workspace 注册记录绝不能替代会话删除或文件系统所有权（参见[决策记录](../../../.agents/notes/implemented/feature/2026-07-27-workspace-registration-deletion.md)）。
