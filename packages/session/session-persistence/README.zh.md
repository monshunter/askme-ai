...
| `delete(id): Promise<boolean>` | 永久删除一个已脱离且已实体化的会话，以及该后端拥有的全部事件数据。协调器会等待最终写入退役，拒绝仍处于实时状态的 id，使保留的 preparation 状态失效，并返回已存记录是否存在。 |
...
| `deleteStored(id)` | 永久删除一个已实体化的会话，并返回其是否存在。协调器已经排除实时所有者，并让该操作与同 id 的写入串行执行。 |
...
- **无保留策略**：调用方选择要删除的确切会话 id；按时间、数量或配额剪枝仍属于部署的带外策略。
