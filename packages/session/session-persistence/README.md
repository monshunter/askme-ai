...
| `delete(id): Promise<boolean>` | Permanently remove one detached materialized Session and all event data owned by this backend. The coordinator waits for final write retirement, rejects a still-live id, invalidates retained preparation state, and returns whether a stored record existed. |
...
| `deleteStored(id)` | Permanently remove one materialized Session and return whether it existed. The coordinator has already excluded a live owner and serialized the operation with writes for that id. |
...
- **No retention policy** — callers choose exact Session ids to delete; age-, count-, and quota-based pruning remain out-of-band deployment policy.
