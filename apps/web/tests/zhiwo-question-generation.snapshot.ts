...
describe('Zhiwo contextual-question browser snapshot', () => {
  ...
  it('generates half the set after the answer and again on manual refresh', async () => {
    ...
    const brandName = sidebar.locator('[data-zhiwo-brand-name]')
    expect(await brandName.isVisible()).toBe(true)
    expect(await brandName.evaluate(node => node.closest('button') === null)).toBe(true)
    expect(await sidebar.locator('[data-zhiwo-brand-mark]').isVisible()).toBe(false)
    const github = sidebar.getByRole('link', { name: '在 GitHub 查看知我AI' })
    expect(await github.getAttribute('href')).toBe('https://github.com/monshunter/askme-ai')
    expect(await github.getAttribute('target')).toBe('_blank')
    expect(await github.getAttribute('rel')).toBe('noopener noreferrer')
